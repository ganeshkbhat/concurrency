...
function _concurrencyMultipleThreads(filename = __filename, numThreads = 1, options = {}, greet = false, close = true) {
    const { Worker, isMainThread, parentPort } = require('worker_threads');

    var messageData = [], childMessageData = [], result = [], workerClusters = [];
    ...
    return new Promise((resolve, reject) => {
        if (isMainThread) {
            ...
            for (let i = 0; i < numThreads; i++) {
                ...
                worker.on("connection", (!!options?.handlers?.parentThreadConnection) ? options.handlers.parentThreadConnection : (e) => {
                ...
                worker.on("message", (!!options?.handlers?.parentThreadMessage) ? options.handlers.parentThreadMessage : function (mainData) {
                    ...
                    if (!!mainData?.closeChild) {
                        ...
                        resolve({ pid: process.pid, threadId: worker.threadId, message: messageData, result: result, event: "message", workerClusters: workerClusters });
                    }
                    // console.log(`demo.threads.js:_concurrencyThreads: Main Thread PID ${process.pid}, filename: ${filename}, data: ${mainData}`);
                });

                worker.on("online", (!!options?.handlers?.parentThreadOnline) ? options.handlers.parentThreadOnline : () => {
                ...
                worker.on("error", (!!options?.handlers?.parentThreadError) ? options.handlers.parentThreadError : (e) => {
                ...
                worker.on("messageerror", (!!options?.handlers?.parentThreadMessageerror) ? options.handlers.parentThreadMessageerror : (e) => {
                ...
                worker.on("close", (!!options?.handlers?.parentThreadClose) ? options.handlers.parentThreadClose : (e) => {
                ...
                worker.on("exit", (!!options?.handlers?.parentThreadExit) ? options.handlers.parentThreadExit : (code) => {
                ...
                workerClusters[i] = worker;
            }

            // });

        } else {
            if (!options["handlers"]?.childThreadMessage) {
                ...
                options.handlers["childThreadMessage"] = function (mainData) {
            ...
            parentPort.on("connection", (!!options?.handlers?.childThreadConnection) ? options.handlers.childThreadConnection : (e) => {
            ...
            parentPort.on("message", (!!options?.handlers?.childThreadMessage) ? options.handlers.childThreadMessage : options?.handlers?.childThreadMessage);

            parentPort.on("error", (!!options?.handlers?.childThreadError) ? options.handlers.childThreadError : (e) => {
            ...
            parentPort.on("messageerror", (!!options?.handlers?.childThreadMessageerror) ? options.handlers.childThreadMessageerror : (e) => {
            ...
            parentPort.on("close", (!!options?.handlers?.childThreadClose) ? options?.handlers?.childThreadClose : (e) => {
